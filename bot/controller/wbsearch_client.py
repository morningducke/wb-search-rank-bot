...
class WBSearchClient:
    ...
    async def _get_search_page(self, query_params: dict, headers: dict = None) -> dict:
    ...
    async def search(self, query: str, item_id: int) -> ProductOnPage | None:
        ...
        # замечание: часто обновляется, так что кол-во может менятся в относительно небольших пределах при разных запросах
        ...
        for page_num in range(2, max_pages + 1):
            ...
            page = await self._get_search_page(query_params=query_params, headers=headers)
